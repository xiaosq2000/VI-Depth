...
def inverse_warp(img, depth, pose, intrinsics, rotation_mode='euler', padding_mode='zeros'):
    ...
    print("intrinsics: ", intrinsics.size())
    print(pose)
...
def inverse_warp_sfm(img, depth, ref_depth, pose, intrinsics, padding_mode='zeros'):
    """
    Inverse warp a source image to the target image plane.
    Args:
        img: the source image (where to sample pixels) -- [B, 3, H, W]
...
def forward_warp(img, depth, pose, intrinsics, upscale=None, rotation_mode='euler', padding_mode='zeros'):
    ...
    """
    print("img warp: ", img.size())
    print("depth warp: ", depth.size())
    check_sizes(depth, 'depth', 'B1HW')
    #check_sizes(pose, 'pose', 'B6')
    check_sizes(intrinsics, 'intrinsics', 'B33')

    # FORWARD WARPING src to target
    ...
    pose_mat = pose #pose_vec2mat(pose, rotation_mode)                    # [B,3,4]
    ...
    for coo, z in zip(pcoords, Z):
        ...
        depth_w.append( 1/torch.sparse.FloatTensor(_idx, _val, torch.Size([hh+1,ww+1])).to_dense()[:-1,:-1] ) # normal depth
    ...
    pose_mat_inv = pose_mat_inv[:3, :3]
    ##INVERSE WARPING
    img_w, iw_val = inverse_warp(img, depth_w, pose_mat_inv, intrinsics)
...
def photometric_loss(projected_img, img1, valid_points):
    # Calculate L1 loss only for valid points
    l1_loss = F.l1_loss(projected_img, img1, reduction='none')
    masked_loss = l1_loss * valid_points.float()
    return masked_loss.sum() / valid_points.float().sum()

def forward_warp_test(img, depth, pose, intrinsics, upscale=None, rotation_mode='euler', padding_mode='zeros'):
    print("img warp: ", img.size())
    print("depth warp: ", depth.size())
    check_sizes(depth, 'depth', 'B1HW')
    check_sizes(intrinsics, 'intrinsics', 'B33')
    ...
    pose_mat = pose  # pose_vec2mat(pose, rotation_mode)  # [B, 3, 4]
    pcoords, Z = cam2pixel(cam_coords, pose_mat[:, :3, :3], pose_mat[:, :, 3:], padding_mode)  # [B, H, W, 2], [B, 1, H, W]

    if torch.__version__[:3] >= "1.3":
        projected_img = F.grid_sample(img, pcoords, padding_mode=padding_mode, align_corners=True)
    else:
        projected_img = F.grid_sample(img, pcoords, padding_mode=padding_mode)

    valid_points = pcoords.abs().max(dim=-1)[0] <= 1

    return projected_img, valid_points

def inverse_warp_test(img, depth, pose, intrinsics, rotation_mode='euler', padding_mode='zeros'):
    print("img: ", img.size())
    print("depth: ", depth.size())
    print("pose: ", pose.size())
    print("intrinsics: ", intrinsics.size())
